...
/**
 * Normalizes any input to an uppercase hex string without prefixes/separators.
 *
 * @param {unknown} value - Raw value that may contain hex characters.
 * @returns {string} Normalized uppercase hex string.
 */
...
/**
 * Extracts payload data from REST responses in different shapes.
 *
 * @param {unknown} payload - REST response payload or nested value.
 * @returns {string|null} Extracted normalized hex payload or `null` if unavailable.
 */
 ...
 *
 * @param {string} hexString - Little-endian hex input.
 * @returns {number} Parsed decimal value or `-1` for invalid input.
 */
 ...
 *
 * @param {number} wtuType - Device type number.
 * @returns {string} Device type label in `id (name)` format.
 */
...
/**
 * Converts a value to a one-byte uppercase hex string.
 *
 * @param {unknown} value - Input value to clamp and encode.
 * @returns {string} Two-character uppercase hex byte.
 */
 ...
 *
 * @param {string} hexString - Raw little-endian hex value of WTU type.
 * @returns {{num: number, text: string}} Decoded WTU type object.
 */
 ...
 *
 * @param {string} hexString - Raw little-endian serial number bytes.
 ...
 *
 * @param {string} hexString - Raw software version bytes.
 ...
 *
 * @param {string} hexString - Raw commissioning timestamp in hex.
 ...
 *
 * @param {string} hexString - Raw runtime counter bytes.
 ...
 *
 * @param {string} hexString - Raw soft water amount bytes.
 ...
 *
 * @param {number} year - Year as decimal number.
 ...
 *
 * @param {string} hexString - Raw total water payload as received in response.
...
function decodeTotalWaterAmount(hexString) {
    ...
    const waterUsageHex = hexString.slice(0, 8);
 ...
 *
 * @param {string} hexString - Raw date/time payload.
 ...
 *
 * @param {string} hexString - Raw status data payload.
...
function decodeStatusData(hexString) {
    ...
    /*
        ...
        a bit nicer:
        i-dos eco status payload (4300)
        The official docs list the general fields, but observed payloads show the
        useful numeric values in these positions:
        - concentration: byte 3
        - dosing amount: bytes 17-18 (little endian)
        - remaining amount in tank: bytes 19-20 (little endian, deciliters)
        - current water flow: bytes 21-22 (little endian, l/h)
        - water consumption: bytes 25-28 (little endian, liters)
     */

    const remainingAmountInTankDeciLiter = hexLeToNumber(hexString.slice(38, 42));
    const remainingAmountInTankLiter = remainingAmountInTankDeciLiter / 10;
    const remainingAmountInTankPercent = Math.min(100, Math.round((remainingAmountInTankLiter / 6) * 100));

    return {
        circuitType: parseInt(hexString.slice(0, 2), 16),
        operatingMode: parseInt(hexString.slice(2, 4), 16),
        concentration: parseInt(hexString.slice(6, 8), 16),
        errorCode: parseInt(hexString.slice(12, 16), 16),
        warnings: parseInt(hexString.slice(16, 20), 16),
        dosingAmount: hexLeToNumber(hexString.slice(34, 38)) / 1000,
        currentWaterFlow: hexLeToNumber(hexString.slice(42, 46)),
        remainingAmountInTank: remainingAmountInTankLiter,
        remainingAmountInTankPercent,
        waterConsumption: hexLeToNumber(hexString.slice(50, 58)),
    };
}
 ...
 *
 * @param {string} hexString - Raw dosage payload.
