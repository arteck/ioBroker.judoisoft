            ...
            if (data.length == 66) {
                if (subIndex !== null) {
                    ...
                    switch (subIndex) {
                        ...
                        case 0:
                            let standby = parseInt(data.slice(0, 2),16);
                            let standbyBinary = (+standby).toString(2);
                            value = standbyBinary;
                            break;

                        // Standby Modus - gibt Stunden zurueck
                        case 9:
                            let standby = data.slice(18, 20);
                            value = parseInt(standby, 16);
                            break;

                        // eingestellter Urlaubsmodus
                        case 18:
                            let urlaubsmodus = data.slice(36, 38);
                            value = parseInt(urlaubsmodus, 16);
                            break;

                        // eingestellter sleepmoduszeit
                        case 19:
                            let urlaubsmodus = data.slice(38, 40);
                            value = parseInt(urlaubsmodus, 16);
                            break;
