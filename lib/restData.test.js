...
describe('Data is parsed correctly', () => {
    ...
    it('total water amount parsing works', () => {
        ...
        // 4-byte little-endian: F9 93 0E 00 -> 0x000E93F9 = 955385 liters = 955.385 m³
        expect(restData.decodeTotalWaterAmount('F9930E00')).to.equal(955.385);

        // error cases
        expect(restData.decodeTotalWaterAmount('')).to.equal(-1);
        expect(restData.decodeTotalWaterAmount('825F')).to.equal(-1);
    });
    ...
    it('yearly statistics parsing works', () => {
        ...
        expect(restData.decodeYearlyStatistics(null)).to.equal(null);
        expect(restData.decodeYearlyStatistics('')).to.equal(null);
    });
});
